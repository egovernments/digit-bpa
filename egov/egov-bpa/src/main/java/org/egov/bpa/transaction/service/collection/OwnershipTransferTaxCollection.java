...
public class OwnershipTransferTaxCollection extends TaxCollection {
    ...
    public void updateOCApplication(final EgDemand demand) {
        final OwnershipTransfer ot = ownershipTransferService.findByDemand(demand);
        if (ot.getStatus().getCode().equals(BpaConstants.APPLICATION_STATUS_SUBMITTED)) {
    ...
    public List<ReceiptDetail> reconstructReceiptDetail(final String billReferenceNumber,
        ...
        final EgBill bill = otBillService.updateBillWithLatest(billID);
