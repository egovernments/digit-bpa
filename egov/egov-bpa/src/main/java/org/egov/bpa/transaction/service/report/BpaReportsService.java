...
public class BpaReportsService {
	...
	public Page<SlotDetailsHelper> searchSlotDetails(final SlotDetailsHelper slotDetailsHelper, final String applicationType) {
		...
		Page<SlotDetail> slotDetails = slotDetailRepository.findAll(BpaReportsSearchSpec.searchReportsSpecification(slotDetailsHelper), pageable);
	...
	public Page<BpaRegisterReportHelper> getBpaRegisterReportDetails(final SearchBpaApplicationForm searchRequest, final List<Long> userIds) {
		...
		Page<BpaApplication> bpaApplications = applicationBpaRepository.findAll(SearchBpaApplnFormSpec.searchBpaRegisterSpecification(searchRequest, positionIds), pageable);
		return new PageImpl<>(new ArrayList<>(buildBpaRegisterReportResponse(bpaApplications, userIds)), pageable, bpaApplications.getTotalElements());
	}

	private Set<BpaRegisterReportHelper> buildBpaRegisterReportResponse(final Page<BpaApplication> bpaApplications, final List<Long> userIds) {
		Set<BpaRegisterReportHelper> bpaRegisterReportHelperList = new HashSet<>();
		for (BpaApplication application : bpaApplications) {
			/*for(Long userId : userIds) {
				if (checkUserInWorkFlow(application, userId)) {*/
					...
					bpaRegister.setApplicationType(application.getServiceType().getDescription().toString());
					...
					if (!application.getBuildingDetail().isEmpty() && application.getBuildingDetail().get(0).getTotalPlintArea() != null) {
					...
					if (application.getStatus().getCode().equalsIgnoreCase(APPLICATION_STATUS_CANCELLED)) {
						bpaRegister.setRejectionReason(bpaNoticeService.buildRejectionReasons(application));
						Assignment rejectedOwner;
						Assignment assignment = bpaWorkFlowService.getApproverAssignmentByDate(application.getCurrentState().getPreviousOwner(), application.getState().getLastModifiedDate());
						...
						if(assignment == null)
							assignment = bpaWorkFlowService.getAssignmentsByPositionAndDate(application.getCurrentState().getPreviousOwner().getId(), application.getState().getLastModifiedDate()).get(0);
						...
						if(lastModifiedBy.getName().equalsIgnoreCase("System")){
							...
							if (assignment != null && assignment.getDesignation().getName().equalsIgnoreCase(SECTION_CLERK)) {
								rejectedOwner = bpaWorkFlowService.getApproverAssignmentByDate(application.getCurrentState().getOwnerPosition(), application.getState().getLastModifiedDate());
								if(application.getCurrentState().getOwnerUser() != null) {
									List<Assignment> assignments = bpaWorkFlowService.getAssignmentByPositionAndUserAsOnDate(application.getCurrentState().getOwnerPosition().getId(), application.getCurrentState().getOwnerUser().getId(), application.getCurrentState().getLastModifiedDate());
									if(!assignments.isEmpty())
								...
								if(rejectedOwner == null)
									rejectedOwner = bpaWorkFlowService.getAssignmentsByPositionAndDate(application.getCurrentState().getOwnerPosition().getId(), application.getState().getLastModifiedDate()).get(0);
							} else {
								rejectedOwner = assignment;
							}
							bpaRegister.setApprvd_rejected_by(rejectedOwner == null ? "N/A" : rejectedOwner.getEmployee().getName().concat(" :: ").concat(rejectedOwner.getDesignation().getName()));
						}
					} else {
						bpaRegister.setFinalApprovalDate(application.getPlanPermissionDate());
						bpaRegister.setApprvd_rejected_by(bpaNoticeService.getApproverName(application).concat(" :: ").concat(bpaNoticeService.getApproverDesignation(bpaWorkFlowService.getAmountRuleByServiceType(application).intValue())));
					}
	...
	private String getNocDetails(final BpaApplication application) {
		StringBuffer nocDetails = new StringBuffer();
