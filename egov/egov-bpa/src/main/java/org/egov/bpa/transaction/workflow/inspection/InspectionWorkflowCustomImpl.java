...
public abstract class InspectionWorkflowCustomImpl implements InspectionWorkflowCustom {
    ...
    public void createCommonWorkflowTransition(final PermitInspectionApplication permitInspection, final WorkflowBean wfBean) {
        ...
        if (null == permitInspection.getInspectionApplication().getState()) {
            ...
            wfmatrix = inspectionWorkflowService.getWfMatrix(permitInspection.getInspectionApplication().getStateType(), null, null, wfBean.getAdditionalRule(),
                    BpaConstants.LPCREATED, null);
            ownerUser = bpaWorkFlowService.getAssignmentsByPositionAndDate(permitInspection.getInspectionApplication().getState().getOwnerPosition().getId(), new Date()).get(0).getEmployee();
            permitInspection.getInspectionApplication().setStatus(getStatusByCurrentMatrxiStatus(wfmatrix));
            permitInspection.getInspectionApplication().transition().progressWithStateCopy()
                    .withSenderName(user.getUsername() + BpaConstants.COLON_CONCATE + user.getName())
                    .withComments(wfBean.getApproverComments()).withStateValue(wfmatrix.getNextState())
                    .withDateInfo(currentDate.toDate()).withOwner(permitInspection.getInspectionApplication().getState().getOwnerPosition()).withOwner(ownerUser)
                    .withNextAction(wfmatrix.getNextAction()).withNatureOfTask(BpaConstants.NATURE_OF_WORK_INSPECTION);
        }  else if (BpaConstants.LPREPLYRECEIVED.equalsIgnoreCase(wfBean.getWorkFlowAction())) {
            List<InspectionLetterToParty> letterToParties = insLetterToPartyService.findByInspectionApplicationOrderByIdDesc(permitInspection.getInspectionApplication());
            StateHistory<Position> stateHistory = bpaWorkFlowService.getStateHistoryToGetLPInitiator(permitInspection.getInspectionApplication().getStateHistory(),
                    letterToParties.get(0).getLetterToParty().getStateForOwnerPosition());
            wfmatrix = workFlowMatrixService
                    .getWorkFlowObjectbyId(bpaWorkFlowService.getPreviousWfMatrixId(permitInspection.getInspectionApplication().getStateHistory(),permitInspection.getInspectionApplication().getState()));
            if (null == wfmatrix)
            	wfmatrix = inspectionWorkflowService.getWfMatrix(permitInspection.getInspectionApplication().getStateType(), null, null, wfBean.getAdditionalRule(),
            			BpaConstants.WF_NEW_STATE, null);
            permitInspection.getInspectionApplication().setStatus(letterToParties.get(0).getLetterToParty().getCurrentApplnStatus());
            permitInspection.getInspectionApplication().transition().progressWithStateCopy()
                    .withSenderName(user.getUsername() + BpaConstants.COLON_CONCATE + user.getName())
                    .withComments(wfBean.getApproverComments()).withStateValue(wfmatrix.getNextState())
