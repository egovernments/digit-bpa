...
public class OwnershipTransferBillService extends BillServiceInterface {
    ...
    @Transactional
    public String generateBill(final OwnershipTransfer ownershipTransfer) {
        String collectXML;
        String currentInstallmentYear;
        final OwnershipTransferBillable otBillable = (OwnershipTransferBillable) context
                .getBean("ownershipTransferBillable");
        ...
        otBillable.setOwnershipTransfer(ownershipTransfer);
        otBillable.getOwnershipTransfer();
        otBillable.setUserId(ApplicationThreadLocals.getUserId());

        otBillable.setReferenceNumber(billRefeNumber.generateBillNumber(currentInstallmentYear));
        otBillable.setBillType(getBillTypeByCode("AUTO"));

        final String billXml = getBillXML(otBillable);
        try {
    ...
    public Criteria getBpaFeeCriteria(Long serviceType, final String feeType, FeeSubType feeSubType) {
        return bpaDemandService.createCriteriaforOTApplicationFeeAmount(serviceType, feeType, feeSubType);
    }
    ...
    public EgDemand createDemand(final OwnershipTransfer ownershipTransfer) {
    	...
    	BigDecimal otAdmissionFee = BigDecimal.ZERO ;
            ...
            for (final BpaFeeMapping feeMap : bpaFeeMap) {
                ...
                feeDetails.put(feeMap.getBpaFeeCommon().getCode(), otAdmissionFee);
            }
        ...
        if (installment != null) {
            ...
            egDemand.setBaseDemand(otAdmissionFee);
    ...
    public EgBill updateBillWithLatest(final Long billId) {
        ...
        otBillable.setOwnershipTransfer(
                ownershipTransferService.findByApplicationNumber(bill.getConsumerId().trim().toUpperCase()));
        final List<EgBillDetails> egBillDetails = getBilldetails(otBillable);
