...
public class PermitRenewalBillService extends BillServiceInterface {
    ...
    @Transactional
    public String generateBill(final PermitRenewal renewal) {
        String collectXML;
        String currentInstallmentYear;
        final PermitRenewalBillable renewalBillable = (PermitRenewalBillable) context
                .getBean("permitRenewalBillable");
        ...
        renewalBillable.setRenewal(renewal);
        renewalBillable.getRenewal();
        renewalBillable.setUserId(ApplicationThreadLocals.getUserId());

        renewalBillable.setReferenceNumber(billRefeNumber.generateBillNumber(currentInstallmentYear));
        renewalBillable.setBillType(getBillTypeByCode("AUTO"));

        final String billXml = getBillXML(renewalBillable);
        try {
    ...
    public EgBill updateBillWithLatest(final Long billId) {
        ...
        renewalBillable.setRenewal(
                permitRenewalService.findByApplicationNumber(bill.getConsumerId().trim().toUpperCase()));
        final List<EgBillDetails> egBillDetails = getBilldetails(renewalBillable);
