...
public class PermitRenewalBillable extends AbstractBillable implements Billable {

    private static final String DISPLAY_MESSAGE = "BPA Permit Renewal Fee Collection";
    private PermitRenewal renewal;
    private Long userId;
    ...
    public String getBillPayee() {
        return renewal.getParent().getOwner().getName();
    }

    @Override
    public String getBillAddress() {
        return renewal.getParent().getOwner() == null ? "Not Mentioned" : renewal.getParent().getOwner().getAddress();
    }

    @Override
    public EgDemand getCurrentDemand() {
        return renewal.getDemand();
    }

    @Override
    public String getEmailId() {
        return renewal.getParent().getOwner().getEmailId();
    }

    @Override
    public List<EgDemand> getAllDemands() {
        final List<EgDemand> demandList = new ArrayList<>();
        demandList.add(renewal.getDemand());
        return demandList;
    }

    @Override
    public EgBillType getBillType() {
        return billType == null ? egBillDAO.getBillTypeByCode("AUTO") : billType;
    }
    ...
    public Long getBoundaryNum() {
        return renewal.getParent().getSiteDetail().get(0) != null
                && renewal.getParent().getSiteDetail().get(0).getAdminBoundary() == null ? 0l
                        : renewal.getParent().getSiteDetail().get(0)
    ...
    public BigDecimal getTotalAmount() {
        ...
        for (final Object object : instVsAmt) {
            ...
            final BigDecimal dmdAmt = BigDecimal.valueOf((Double) ddObject[2]);
            BigDecimal collAmt = BigDecimal.ZERO;
            if (ddObject[2] != null)
                collAmt = BigDecimal.valueOf((Double) ddObject[3]);
    ...
    public String getDescription() {
        return "Permit Renewal Application Number: " + renewal.getApplicationNumber();
    }
    ...
    public String getConsumerId() {
        return renewal.getApplicationNumber();
    }

    @Override
    public String getConsumerType() {
        return renewal.getParent().getServiceType().getCode();
    }
    ...
    public PermitRenewal getRenewal() {
        return renewal;
    }

    public void setRenewal(PermitRenewal renewal) {
