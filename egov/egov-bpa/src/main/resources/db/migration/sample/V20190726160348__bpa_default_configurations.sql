--Tenant list
insert into state.eg_tenant (id, name, code, createdby, createddate, lastmodifieddate, lastmodifiedby, url, version) select nextval('seq_eg_tenant'), 'Digit', 'ulb', 1, now(), now(), 1, 'http://ulb.egovernments.org:8080/', 0 where not exists (select id from state.eg_tenant where code='ulb');
update state.eg_tenant set url='http://ulb.egovernments.org:8080/' where code='ulb';
