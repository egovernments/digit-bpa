...
public class Far extends  FeatureProcess {
    ...
    public Plan process(Plan pl) {
        ...
        for (Block blk : pl.getBlocks())
        {
            ...
            for (Floor flr : building.getFloors())
            {
                for (Occupancy occupancy : flr.getOccupancies())
                {
                    ...
                    bltUpArea = bltUpArea.add(occupancy.getBuiltUpArea() == null ? BigDecimal.valueOf(0) : occupancy.getBuiltUpArea());
                    ...
                    existingBltUpArea = existingBltUpArea.add(occupancy.getExistingBuiltUpArea() == null ? BigDecimal.valueOf(0) : occupancy.getExistingBuiltUpArea());
                        ...
                        for (Occupancy occupancy : flr.getOccupancies())
                        {
                            if (occupancy.getType().equals(occupancyType)) {
                                ...
                                blockWiseExistingBuiltupArea = blockWiseExistingBuiltupArea
                                        .add(occupancy.getExistingBuiltUpArea() == null ? BigDecimal.valueOf(0) : occupancy.getExistingBuiltUpArea());
            ...
            if (!listOfOccupanciesOfAParticularblock.isEmpty()) {
                ...
                for (Occupancy occupancy : listOfOccupanciesOfAParticularblock) {
                   // System.out.println("occupancy :" + occupancy.getType());
                    if (occupancy.getType() != null) {
                        ...
                        if (occupancy.getType().equals(OccupancyType.OCCUPANCY_A1) || occupancy.getType().equals(OccupancyType.OCCUPANCY_A4)) {
                ...
                for (Occupancy occupancy : listOfOccupanciesOfAParticularblock) {
                    if (occupancy.getType() != null) {
                        ...
                        if (occupancy.getType().equals(OccupancyType.OCCUPANCY_A1) || occupancy.getType().equals(OccupancyType.OCCUPANCY_A4) ||
                                occupancy.getType().equals(OccupancyType.OCCUPANCY_F) || occupancy.getType().equals(OccupancyType.OCCUPANCY_F1) ||
                                occupancy.getType().equals(OccupancyType.OCCUPANCY_F2) || occupancy.getType().equals(OccupancyType.OCCUPANCY_F3)
        ...
        if(pl.getPlot().getArea().doubleValue() > 0)
            far = pl.getVirtualBuilding().getTotalFloorArea().divide(pl.getPlot().getArea(), DECIMALDIGITS_MEASUREMENTS, ROUNDMODE_MEASUREMENTS);
	...
	private void calculateFar(Plan pl, OccupancyType mostRestrictiveOccupancy, BigDecimal far) {
        ...
        if (pl.getPlot().getArea().doubleValue() >= 5000) {
			...
			for(Occupancy occ:pl.getOccupancies()) {
                weightedAreaWOAddnlFee = weightedAreaWOAddnlFee.add(occ.getFloorArea().multiply(getPermissibleFar(occ.getType())));
                weightedAreaWithAddnlFee = weightedAreaWithAddnlFee.add(occ.getFloorArea().multiply(getMaxPermissibleFar(occ.getType())));
			}
		    ...
		    if (mostRestrictiveOccupancy != null) {
		        switch (mostRestrictiveOccupancy) {
		            case OCCUPANCY_A1:
		            case OCCUPANCY_A4:
		                processFar(pl,mostRestrictiveOccupancy.getOccupancyTypeVal(), pl.getFar(), four, three,scrutinyDetail, null);
		                break;
		            case OCCUPANCY_A2:
		                processFar(pl,mostRestrictiveOccupancy.getOccupancyTypeVal(), pl.getFar(), four, twoPointFive,scrutinyDetail, null);
		                break;
		          //  case OCCUPANCY_B:    
		            case OCCUPANCY_B1:
		            case OCCUPANCY_B2:
		            case OCCUPANCY_B3:
		                processFar(pl,mostRestrictiveOccupancy.getOccupancyTypeVal(), pl.getFar(), three, twoPointFive,scrutinyDetail, null);
		                break;
		            case OCCUPANCY_C:    
		            case OCCUPANCY_C1:
		            case OCCUPANCY_C2:
		            case OCCUPANCY_C3:
		                processFar(pl,mostRestrictiveOccupancy.getOccupancyTypeVal(), pl.getFar(), threePointFive, twoPointFive,scrutinyDetail, null);
		                break;
		            case OCCUPANCY_D:
		            case OCCUPANCY_D1:
		            case OCCUPANCY_D2:
		                processFar(pl,mostRestrictiveOccupancy.getOccupancyTypeVal(), pl.getFar(), twoPointFive, onePointFive,scrutinyDetail, null);
		                break;
		            case OCCUPANCY_E:
		                processFar(pl,mostRestrictiveOccupancy.getOccupancyTypeVal(), pl.getFar(), four, three,scrutinyDetail, null);
		                break;
		            ...
		            case OCCUPANCY_F4:
		                ...
		                break;
		            case OCCUPANCY_G1:
		                processFar(pl,mostRestrictiveOccupancy.getOccupancyTypeVal(), pl.getFar(), twoPointFive, twoPointFive,scrutinyDetail, null);
		                break;
		            case OCCUPANCY_G2:
		                processFar(pl,mostRestrictiveOccupancy.getOccupancyTypeVal(), pl.getFar(), four, threePointFive,scrutinyDetail, null);
		                break;
		            case OCCUPANCY_H:
		                processFar(pl,mostRestrictiveOccupancy.getOccupancyTypeVal(), pl.getFar(), four, three,scrutinyDetail, null);
		                break;
		            case OCCUPANCY_I1:
		                ...
		                break;
		            case OCCUPANCY_I2:
		                processFar(pl,mostRestrictiveOccupancy.getOccupancyTypeVal(), pl.getFar(), onePointFive, onePointFive,scrutinyDetail, null);
		                break;
		            default:
		                break;
	...
	private void validate2(Plan pl, Block blk, Floor flr, Occupancy occupancy) {
		...
		if (occupancy.getExistingBuiltUpArea() != null && occupancy.getExistingBuiltUpArea().compareTo(BigDecimal.valueOf(0)) < 0) {
		...
		occupancy.setExistingFloorArea((occupancy.getExistingBuiltUpArea() == null ? BigDecimal.ZERO : occupancy.getExistingBuiltUpArea()).subtract(
    ...
    private void processFar(Plan pl, String occupancyType,BigDecimal far, BigDecimal upperLimit, BigDecimal additionFeeLimit,ScrutinyDetail scrutinyDetail, String desc) {
