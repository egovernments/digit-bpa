...
public class RearYardService extends GeneralRule {
								...
								if (occupancy.getType().equals(OCCUPANCY_A1)
									...
									if (buildingHeight.compareTo(BigDecimal.valueOf(10)) <= 0
										...
										checkRearYardUptoTenMts(pl, block.getBuilding(), block, setback.getLevel(),
												plot, REAR_YARD_DESC, min, mean, occupancy.getType(), rearYardResult,
												buildingHeight);
										...
										checkRearYardUptoToTweleveMts(setback, block.getBuilding(), pl, block,
												...
												occupancy.getType(), rearYardResult, errors);
									...
									checkRearYardForIndustrial(setback, block.getBuilding(), pl, block,
											setback.getLevel(), plot, REAR_YARD_DESC, min, mean, occupancy.getType(),
											rearYardResult);
								} else {
	...
	private Boolean checkRearYardUptoTenMts(final Plan pl, Building building, Block block, Integer level,
		...
		if (mostRestrictiveOccupancy.equals(OCCUPANCY_A1)) {
			valid = residentialUptoTenMts(block, level, min, mean, mostRestrictiveOccupancy, rearYardResult, subRule,
	...
	private Boolean residentialUptoTenMts(Block block, Integer level, final BigDecimal min, final BigDecimal mean,
			final OccupancyType mostRestrictiveOccupancy, RearYardResult rearYardResult, String subRule, String rule,
			BigDecimal minVal, BigDecimal meanVal, BigDecimal depthOfPlot, Boolean valid) {
	...
	private Boolean checkRearYardUptoToTweleveMts(SetBack setback, Building building, final Plan pl, Block block,
			...
			final OccupancyType mostRestrictiveOccupancy, RearYardResult rearYardResult,
		...
		if (mostRestrictiveOccupancy.equals(OCCUPANCY_A1)) {
			valid = residentialUptoTwelveMts(block, level, min, mean, mostRestrictiveOccupancy, rearYardResult, subRule,
					rule, minVal, meanVal, depthOfPlot, valid, errors, pl);
	...
	private Boolean processRearYardForIndustrial(Block block, Integer level, final BigDecimal min,
			final BigDecimal mean, final OccupancyType mostRestrictiveOccupancy, RearYardResult rearYardResult,
	...
	private Boolean allOccupancyForHighRise(Block block, Integer level, final BigDecimal min, final BigDecimal mean,
			final OccupancyType mostRestrictiveOccupancy, RearYardResult rearYardResult, String subRule, String rule,
			BigDecimal minVal, BigDecimal meanVal, BigDecimal blockBuildingHeight) {
		Boolean valid = false;
...
//IT,ITES
	...
	private void compareRearYardResult(String blockName, BigDecimal min, BigDecimal mean,
			OccupancyType mostRestrictiveOccupancy, RearYardResult rearYardResult, Boolean valid, String subRule,
