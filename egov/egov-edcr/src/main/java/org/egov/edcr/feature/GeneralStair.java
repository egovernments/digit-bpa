/*
 * package org.egov.edcr.feature; import java.math.BigDecimal; import java.util.Date; import java.util.HashMap; import
 * java.util.LinkedHashMap; import java.util.List; import java.util.Map; import org.apache.log4j.Logger; import
 * org.egov.common.entity.edcr.Block; import org.egov.common.entity.edcr.Floor; import org.egov.common.entity.edcr.Measurement;
 * import org.egov.common.entity.edcr.OccupancyTypeHelper; import org.egov.common.entity.edcr.Plan; import
 * org.egov.common.entity.edcr.Result; import org.egov.common.entity.edcr.ScrutinyDetail; import
 * org.egov.edcr.constants.DxfFileConstants; import org.egov.edcr.utility.DcrConstants; import org.egov.edcr.utility.Util; import
 * org.springframework.context.i18n.LocaleContextHolder; import org.springframework.stereotype.Service;
 * @Service public class GeneralStair extends FeatureProcess { private static final Logger LOG =
 * Logger.getLogger(GeneralStair.class); private static final String FLOOR = "Floor"; private static final String RULE42_5_II =
 * "42-5-ii"; // private static final String RULE42 = "42"; // private static final String EXPECTED_WIDTH = "0.75"; // private
 * static final String EXPECTED_LINE = "0.75"; // private static final String EXPECTED_TREAD = "0.15"; private static final String
 * EXPECTED_NO_OF_RISE = "12"; // private static final String EXPECTED_TREAD_HIGHRISE = "0.2"; private static final String
 * NO_OF_RISE_DESCRIPTION = "Maximum no of rises required per flight for general stair %s"; private static final String
 * WIDTH_DESCRIPTION = "Minimum width for general stair %s"; private static final String TREAD_DESCRIPTION =
 * "Minimum tread for general stair %s"; // private static final String LINE_DESCRIPTION = "Minimum length of line for // fire
 * stair %s flight layer"; // private static final String HEIGHT_FLOOR_DESCRIPTION = "Height of floor in // layer "; private
 * static final String NO_OF_RISES = "Number of rises "; private static final String FLIGHT_POLYLINE_NOT_DEFINED_DESCRIPTION =
 ...
 * blk: for (Block block : planDetail.getBlocks()) { int generalStairCount = 0; if (block.getBuilding() != null) { if
 ...
 * block.getNumber() + "_" + "General Stair - Width"); ScrutinyDetail scrutinyDetail3 = new ScrutinyDetail();
 ...
 * scrutinyDetail3.addColumnHeading(6, STATUS); scrutinyDetail3.setKey("Block_" + block.getNumber() + "_" +
 * "General Stair - Tread"); ScrutinyDetail scrutinyDetailRise = new ScrutinyDetail(); scrutinyDetailRise.addColumnHeading(1,
 * RULE_NO); scrutinyDetailRise.addColumnHeading(2, FLOOR); scrutinyDetailRise.addColumnHeading(3, DESCRIPTION);
 ...
 * "General Stair - Number of rises"); OccupancyTypeHelper mostRestrictiveOccupancyType = planDetail.getVirtualBuilding()
 * .getMostRestrictiveFarHelper(); String occupancyType = mostRestrictiveOccupancy != null ?
 ...
 * List<org.egov.common.entity.edcr.GeneralStair> generalStairs = floor.getGeneralStairs(); int size = generalStairs.size();
 * generalStairCount = generalStairCount + size; if (size != 0) { for (org.egov.common.entity.edcr.GeneralStair generalStair :
 * generalStairs) { List<Measurement> flightPolyLines = generalStair.getFlightPolyLines(); List<BigDecimal> flightLengths =
 * generalStair.getLengthOfFlights(); List<BigDecimal> flightWidths = generalStair.getWidthOfFlights(); BigDecimal noOfRises =
 * generalStair.getNoOfRises(); Boolean flightPolyLineClosed = generalStair.getFlightPolyLineClosed(); BigDecimal minTread =
 * BigDecimal.ZERO; BigDecimal minFlightWidth = BigDecimal.ZERO; String flightLayerName =
 * String.format(DxfFileConstants.LAYER_STAIR_FLIGHT_FLOOR, block.getNumber(), floor.getNumber(), generalStair.getNumber()); if
 * (!floor.getTerrace()) { if (flightPolyLines != null && flightPolyLines.size() > 0) { if (flightPolyLineClosed) { if
 * (flightWidths != null && flightWidths.size() > 0) { minFlightWidth = validateWidth(planDetail, scrutinyDetail2, floor, block,
 * typicalFloorValues, generalStair, flightWidths, minFlightWidth, mostRestrictiveOccupancyType); } else {
 * errors.put("Flight PolyLine width" + flightLayerName, FLIGHT_WIDTH_DEFINED_DESCRIPTION + flightLayerName);
 * planDetail.addErrors(errors); } (Total length of polygons in layer BLK_n_FLR_i_STAIR_k_FLIGHT) / (Number of rises - number of
 * polygons in layer BLK_n_FLR_i_STAIR_k_FLIGHT - number of lines in layer BLK_n_FLR_i_STAIR_k_FLIGHT) if (flightLengths != null
 * && flightLengths.size() > 0) { try { minTread = validateTread(planDetail, errors, block, scrutinyDetail3, floor,
 * typicalFloorValues, generalStair, flightLengths, minTread, mostRestrictiveOccupancyType); } catch (ArithmeticException e) {
 * LOG.info("Denominator is zero"); } } else { errors.put("Flight PolyLine length" + flightLayerName,
 * FLIGHT_LENGTH_DEFINED_DESCRIPTION + flightLayerName); planDetail.addErrors(errors); } if (noOfRises.compareTo(BigDecimal.ZERO)
 * > 0) { try { validateNoOfRises(planDetail, errors, block, scrutinyDetailRise, floor, typicalFloorValues, generalStair,
 * noOfRises); } catch (ArithmeticException e) { LOG.info("Denominator is zero"); } } else { String layerName = String.format(
 * DxfFileConstants.LAYER_STAIR_FLIGHT_FLOOR, block.getNumber(), floor.getNumber(), generalStair.getNumber());
 ...
 * !floor.getTerrace()) { fireStair.setGeneralStair(true); } } } } if (block.getBuilding().getFloors().size() > 1 &&
 * generalStairCount == 0) { errors.put("General Stair not defined in blk " + block.getNumber(),
 * "General Stair not defined in block " + block.getNumber() + ", it is mandatory for building with floors more than one.");
 * planDetail.addErrors(errors); } boolean isAbuting = abutingList.stream().anyMatch(aBoolean -> aBoolean == true); if
 * (occupancyType != null) { if (occupancyType.equalsIgnoreCase("RESIDENTIAL") && floorSize.compareTo(BigDecimal.valueOf(3)) > 0)
 * { if (fireStairCount > 0) { setReportOutputDetails(planDetail, RULE42, String.format(DcrConstants.RULE114, block.getNumber()),
 ...
 * block, Map<String, Object> typicalFloorValues, org.egov.common.entity.edcr.GeneralStair generalStair, List<BigDecimal>
 * flightWidths, BigDecimal minFlightWidth, OccupancyTypeHelper mostRestrictiveOccupancyType) { BigDecimal flightPolyLine =
 ...
 * minimumWidth = getRequiredWidth(block, mostRestrictiveOccupancyType); if (minFlightWidth.compareTo(minimumWidth) >= 0) { valid
 * = true; } String value = typicalFloorValues.get("typicalFloors") != null ? (String) typicalFloorValues.get("typicalFloors") :
 * " floor " + floor.getNumber(); if (valid) { setReportOutputDetailsFloorStairWise(planDetail, RULE42_5_II, value,
 * String.format(WIDTH_DESCRIPTION, generalStair.getNumber()), minimumWidth.toString(), String.valueOf(minFlightWidth),
 ...
 * org.egov.common.entity.edcr.GeneralStair generalStair, List<BigDecimal> flightLengths, BigDecimal minTread, OccupancyTypeHelper
 * mostRestrictiveOccupancyType) { BigDecimal totalLength = flightLengths.stream().reduce(BigDecimal.ZERO, BigDecimal::add);
 * totalLength = Util.roundOffTwoDecimal(totalLength); BigDecimal requiredTread = getRequiredTread(mostRestrictiveOccupancyType);
 * if (generalStair.getNoOfRises() != null) { BigDecimal denominator =
 ...
 * BigDecimal.valueOf(flightLengths.size()); if (generalStair.getNoOfRises().compareTo(noOfFlights) > 0) { BigDecimal denominator
 * = generalStair.getNoOfRises().subtract(noOfFlights); minTread = totalLength.divide(denominator,
 ...
 * String.format(TREAD_DESCRIPTION, generalStair.getNumber()), requiredTread.toString(), String.valueOf(minTread),
 * Result.Accepted.getResultVal(), scrutinyDetail3); } else { setReportOutputDetailsFloorStairWise(planDetail, RULE42_5_II, value,
 * String.format(TREAD_DESCRIPTION, generalStair.getNumber()), requiredTread.toString(), String.valueOf(minTread),
 * Result.Not_Accepted.getResultVal(), scrutinyDetail3); } } } else { if (generalStair.getNoOfRises().compareTo(BigDecimal.ZERO) >
 * 0) { String flightLayerName = String.format(DxfFileConstants.LAYER_STAIR_FLIGHT_FLOOR, block.getNumber(), floor.getNumber(),
 * generalStair.getNumber()); errors.put("NoOfRisesCount" + flightLayerName,
 ...
 * org.egov.common.entity.edcr.GeneralStair generalStair, BigDecimal noOfRises) { boolean valid = false; if (!(Boolean)
 * typicalFloorValues.get("isTypicalRepititiveFloor")) { if
 ...
 * floor.getNumber(); if (valid) { setReportOutputDetailsFloorStairWise(planDetail, RULE42_5_II, value,
 * String.format(NO_OF_RISE_DESCRIPTION, generalStair.getNumber()), EXPECTED_NO_OF_RISE, String.valueOf(noOfRises),
 * Result.Accepted.getResultVal(), scrutinyDetail3); } else { setReportOutputDetailsFloorStairWise(planDetail, RULE42_5_II, value,
 * String.format(NO_OF_RISE_DESCRIPTION, generalStair.getNumber()), EXPECTED_NO_OF_RISE, String.valueOf(noOfRises),
