...
import org.egov.edcr.entity.DcrReportPlanDetail;
...
import org.egov.edcr.entity.VirtualBuildingReport;
...
public class PlanReportService {
    ...
    public InputStream generateDynamicReport(Plan plan,  EdcrApplication dcrApplication) {
        ...
        try {
            ...
            for (String cmnFeature : common) {
                i++;
                drb.addConcatenatedReport(getSub(allMap.get(cmnFeature), i, i + "." + cmnFeature, allMap.get(cmnFeature).getHeading(), allMap.get(cmnFeature).getSubHeading(), cmnFeature));
            ...
            for (String blkName : blocks.keySet()) {
                ...
                for (String blkFeature : blocks.get(blkName)) {
                    j++;
                    drb.addConcatenatedReport(getSub(allMap.get(blkName + blkFeature), j, j + "." + blkFeature, allMap.get(blkName + blkFeature).getHeading(), allMap.get(blkName + blkFeature).getSubHeading(), blkName + blkFeature));
                    ...
                    if (allMap.get(blkName + blkFeature).getRemarks() != null) {
                        drb.addConcatenatedReport(createFooterSubreport("Remarks :  " + allMap.get(blkName + blkFeature).getRemarks(),
        ...
        try {
            ...
            if (dcrReportBlockDetail.getBlockNo() != null) {
                if (isProposed) {
                    ...
                    String coveredAreaText = "1. Covered Area is " + (dcrReportBlockDetail.getCoverageArea() != null ? dcrReportBlockDetail.getCoverageArea().setScale(DcrConstants.DECIMALDIGITS_MEASUREMENTS,
                            DcrConstants.ROUNDMODE_MEASUREMENTS) : BigDecimal.ZERO) + " m²";

                    String blgHgtText = "2. Height of building is " + (dcrReportBlockDetail.getBuildingHeight() != null ? dcrReportBlockDetail.getBuildingHeight().setScale(DcrConstants.DECIMALDIGITS_MEASUREMENTS,
                            DcrConstants.ROUNDMODE_MEASUREMENTS) : BigDecimal.ZERO) + " m";
            ...
            sub.setStyle(style);
            if (isProposed) {
                sub.setDatasource(new DJDataSource("Block No " + dcrReportBlockDetail.getBlockNo(), DJConstants.DATA_SOURCE_ORIGIN_PARAMETER, 0));
            } else
                sub.setDatasource(new DJDataSource("Existing Block No " + dcrReportBlockDetail.getBlockNo(), DJConstants.DATA_SOURCE_ORIGIN_PARAMETER, 0));
            sub.setLayoutManager(new ClassicLayoutManager());
            return sub;
        } catch (ColumnBuilderException e) {
            e.printStackTrace();
    ...
    public InputStream generateReport(Plan plan, EdcrApplication dcrApplication) {
        ...
        if (plan.getVirtualBuilding() != null && plan.getVirtualBuilding().getOccupancies() != null && plan.getVirtualBuilding().getOccupancies().size() > 0) {
        ...
        valuesMap.put("totalFloorArea", plan.getVirtualBuilding() != null
                ? plan.getVirtualBuilding().getTotalFloorArea() : BigDecimal.valueOf(0));
        valuesMap.put("totalBuiltUpArea", plan.getVirtualBuilding() != null
                ? plan.getVirtualBuilding().getTotalBuitUpArea() : BigDecimal.valueOf(0));
        valuesMap.put("electricLineVoltage", voltages);
        valuesMap.put("blockCount", plan.getBlocks() != null && !plan.getBlocks().isEmpty()
                ? plan.getBlocks().size() : 0);
        ...
        if (clientSpecificSubReport) {
            ...
            /*
             * List<PlanRule> planRules =
			 * planRuleService.findRulesByPlanDetail(planDetail);
			 *
			 * for (PlanRule pl : planRules) { String rules = pl.getRules();
			 * String[] ruleSet = rules.split(","); for (String s : ruleSet) {
			 * String ruleName = "rule" + s; if(LOG.isDebugEnabled())
			 * LOG.debug(s); Object ruleBean = getRuleBean(ruleName); if
			 * (ruleBean != null) { GeneralRule bean = (GeneralRule) ruleBean;
			 * if (bean != null){ reportStatus =
			 * bean.generateRuleReport(planDetail, drb, valuesMap,
			 * reportStatus); if(!reportStatus) { finalReportStatus=false; }
			 *
			 * } } else LOG.error("Skipping rule " + ruleName +
			 * "Since rule cannot be injected"); } }
			 */
        }
                        ...
                        for (Floor floor : floors) {
                            ...
                            if (occupancies.size() > 0) {
                                ...
                                for (Occupancy occupancy : occupancies) {
                                    ...
                                    dcrReportFloorDetail.setFloorNo(floor.getTerrace() ? "Terrace" : floor.getNumber().toString());
                                    ...
                                    dcrReportFloorDetail.setBuiltUpArea(
                                            occupancy.getExistingBuiltUpArea().compareTo(BigDecimal.ZERO) > 0 ?
                                                    occupancy.getBuiltUpArea().subtract(occupancy.getExistingBuiltUpArea()) : occupancy.getBuiltUpArea());
                                    dcrReportFloorDetail.setFloorArea(occupancy.getExistingFloorArea().compareTo(BigDecimal.ZERO) > 0 ?
                                            occupancy.getFloorArea().subtract(occupancy.getExistingFloorArea()) : occupancy.getFloorArea());
                                    dcrReportFloorDetail.setCarpetArea(occupancy.getExistingCarpetArea().compareTo(BigDecimal.ZERO) > 0 ?
                                            occupancy.getCarpetArea().subtract(occupancy.getExistingCarpetArea()) : occupancy.getCarpetArea());
    ...
    private List<DcrReportBlockDetail> buildBlockWiseExistingInfo(Plan plan) {
        ...
        if (blocks.size() > 0) {
            ...
            for (Block block : blocks) {
                ...
                Building building = block.getBuilding();
                if (building != null && building.getTotalExistingBuiltUpArea() != null && building.getTotalExistingBuiltUpArea().compareTo(BigDecimal.ZERO) > 0 &&
