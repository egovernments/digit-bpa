...
public class Parking extends FeatureProcess {
	...
	public void processParking(Plan pl) {
		...
		if (totalProvidedCarParkArea.doubleValue() == 0) {
			...
			setReportOutputDetails(pl, SUB_RULE_40_2, SUB_RULE_40_2_DESCRIPTION, requiredCarParkArea + SQMTRS,
					totalProvidedCarParkArea + SQMTRS, Result.Not_Accepted.getResultVal());
		} else {
			setReportOutputDetails(pl, SUB_RULE_40_2, SUB_RULE_40_2_DESCRIPTION, requiredCarParkArea + SQMTRS,
					totalProvidedCarParkArea + SQMTRS, Result.Accepted.getResultVal());
		}
		if (requiredVisitorParkArea > 0 && providedVisitorParkArea.doubleValue() < requiredVisitorParkArea) {
			setReportOutputDetails(pl, SUB_RULE_40_10, SUB_RULE_40_10_DESCRIPTION, requiredVisitorParkArea + SQMTRS,
					providedVisitorParkArea + SQMTRS, Result.Not_Accepted.getResultVal());
		} else if (requiredVisitorParkArea > 0) {
			setReportOutputDetails(pl, SUB_RULE_40_10, SUB_RULE_40_10_DESCRIPTION, requiredVisitorParkArea + SQMTRS,
					providedVisitorParkArea + SQMTRS, Result.Accepted.getResultVal());
		}
	 ...
	 * parkSlotAreaInFrontYard + DcrConstants.SQMTRS, Result.Not_Accepted,
	 * null)); } private BigDecimal validateParkingSlotsAreWithInYard(Plan Plan,
	 * Polygon yardPolygon) { BigDecimal area = BigDecimal.ZERO; for
	 * (Measurement parkingSlot : Plan.getParkingDetails().getCars()) { Iterator
	 * parkSlotIterator = parkingSlot.getPolyLine().getVertexIterator(); while
	...
	private void checkDimensionForCarParking(Plan pl, ParkingHelper helper) {
		 ...
		 * = ProcessHelper.getPolygon(setBack.getFrontYard().getPolyLine());
		 * BigDecimal parkSlotAreaInFrontYard =
		 * validateParkingSlotsAreWithInYard(Plan, frontYardPolygon); BigDecimal
		 * maxAllowedArea =
		 ...
		 * buildResultForYardValidation(Plan, parkSlotAreaInFrontYard,
		 * maxAllowedArea, "front yard space"); } } if (setBack.getRearYard() !=
		 * null && setBack.getRearYard().getPresentInDxf()) { Polygon
		 * rearYardPolygon =
		 * ProcessHelper.getPolygon(setBack.getRearYard().getPolyLine());
		 * BigDecimal parkSlotAreaInRearYard =
		 * validateParkingSlotsAreWithInYard(Plan, rearYardPolygon); BigDecimal
		 * maxAllowedArea =
		 * setBack.getRearYard().getArea().divide(BigDecimal.valueOf(2),
		 * DcrConstants.DECIMALDIGITS_MEASUREMENTS, RoundingMode.HALF_UP); if
		 * (parkSlotAreaInRearYard.compareTo(maxAllowedArea) > 0) {
		 * buildResultForYardValidation(Plan, parkSlotAreaInRearYard,
		 * maxAllowedArea, "rear yard space"); } } if (setBack.getSideYard1() !=
		 * null && setBack.getSideYard1().getPresentInDxf()) { Polygon
		 * sideYard1Polygon =
		 * ProcessHelper.getPolygon(setBack.getSideYard1().getPolyLine());
		 * BigDecimal parkSlotAreaInSideYard1 =
		 * validateParkingSlotsAreWithInYard(Plan, sideYard1Polygon); BigDecimal
		 * maxAllowedArea =
		 * setBack.getSideYard1().getArea().divide(BigDecimal.valueOf(2),
		 * DcrConstants.DECIMALDIGITS_MEASUREMENTS, RoundingMode.HALF_UP); if
		 * (parkSlotAreaInSideYard1.compareTo(maxAllowedArea) > 0) {
		 * buildResultForYardValidation(Plan, parkSlotAreaInSideYard1,
		 * maxAllowedArea, "side yard1 space"); } } if (setBack.getSideYard2()
		 * != null && setBack.getSideYard2().getPresentInDxf()) { Polygon
		 * sideYard2Polygon =
		 * ProcessHelper.getPolygon(setBack.getSideYard2().getPolyLine());
		 * BigDecimal parkSlotAreaInFrontYard =
		 * validateParkingSlotsAreWithInYard(Plan, sideYard2Polygon); BigDecimal
		 * maxAllowedArea =
