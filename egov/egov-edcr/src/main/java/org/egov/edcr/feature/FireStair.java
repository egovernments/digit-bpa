...
public class FireStair extends FeatureProcess {
    ...
    public Plan process(Plan planDetail) {
        ...
        blk: for (Block block : planDetail.getBlocks()) {
            ...
            if (block.getBuilding() != null) {
                    ...
                    if (!floor.getTerrace()) {
                        ...
                        if (!fireStairs.isEmpty()) {
                            for (org.egov.common.entity.edcr.FireStair fireStair : fireStairs) {
                                ...
                                if (!landings.isEmpty()) {
                                    validateLanding(planDetail, scrutinyDetailLanding, floor, typicalFloorValues, fireStair,
                                            landings);
                                } else {
                                    errors.put(
                ...
                if (block.getBuilding().getBuildingHeight().compareTo(BigDecimal.valueOf(15)) > 0
                        && !fireStairAbsent.isEmpty()) {
                    for (String error : fireStairAbsent) {
                        errors.put("Fire Stair " + error,
                                "Fire stair not defined in " + error);
                        planDetail.addErrors(errors);
                    }
                }
                ...
                if (block.getBuilding().getBuildingHeight().compareTo(BigDecimal.valueOf(15)) > 0
                        && fireStairCount == 0) {
                    errors.put("FireStair not defined in blk " + block.getNumber(),
                            "FireStair not defined in block " + block.getNumber()
                                    + ", it is mandatory for building with height more than 15m.");
                    planDetail.addErrors(errors);
                }
    ...
    private void validateFlight(Plan planDetail, HashMap<String, String> errors, Block block, ScrutinyDetail scrutinyDetail2,
            ...
            Floor floor, Map<String, Object> typicalFloorValues, org.egov.common.entity.edcr.FireStair fireStair) {
        for (Flight flight : fireStair.getFlights()) {
            ...
            String flightLayerName = String.format(DxfFileConstants.LAYER_FIRESTAIR_FLIGHT,
                    block.getNumber(), floor.getNumber(), fireStair.getNumber(), flight.getNumber());
                    ...
                    if (flightPolyLineClosed) {
                        ...
                        if (flightLengths != null && flightLengths.size() > 0) {
                            try {
                                minTread = validateTread(planDetail, errors, block, scrutinyDetail3,
                                        floor, typicalFloorValues, fireStair, flight, flightLengths,
                                        minTread,
                                        mostRestrictiveOccupancyType);
                        ...
                        if (noOfRises.compareTo(BigDecimal.ZERO) > 0) {
                            ...
                            String layerName = String.format(
                                    DxfFileConstants.LAYER_FIRESTAIR_FLIGHT, block.getNumber(),
             ...
             * Util.roundOffTwoDecimal(BigDecimal.valueOf(0.75)); if (lineLength.compareTo(minLineLength) >= 0) { valid = true; }
             * String value = typicalFloorValues.get("typicalFloors") != null ? (String) typicalFloorValues.get("typicalFloors") :
             * " floor " + floor.getNumber(); if (valid) setReportOutputDetailsFloorStairWise(planDetail, RULE114, value,
             * String.format(LINE_DESCRIPTION, fireStair.getNumber()), EXPECTED_LINE, String.valueOf(lineLength),
             * Result.Accepted.getResultVal(), scrutinyDetail6); else setReportOutputDetailsFloorStairWise(planDetail, RULE114,
             * value, String.format(LINE_DESCRIPTION, fireStair.getNumber()), EXPECTED_LINE, String.valueOf(lineLength),
    ...
    private BigDecimal validateWidth(Plan planDetail, ScrutinyDetail scrutinyDetail2, Floor floor, Block block,
        ...
        if (!(Boolean) typicalFloorValues.get("isTypicalRepititiveFloor")) {
            ...
            if (valid) {
                setReportOutputDetailsFloorStairWise(planDetail, RULE42_5_II, value,
                        String.format(WIDTH_DESCRIPTION, fireStair.getNumber(), flight.getNumber()), minimumWidth.toString(),
     ...
     * (mostRestrictiveOccupancyType.getType() != null &&
     * DxfFileConstants.A_AF_GH.equalsIgnoreCase(mostRestrictiveOccupancyType.getType().getCode())) { return
     * BigDecimal.valueOf(0.75); } else if (mostRestrictiveOccupancyType.getType() != null &&
    ...
    private BigDecimal validateTread(Plan planDetail, HashMap<String, String> errors, Block block,
            ...
            org.egov.common.entity.edcr.FireStair fireStair, Flight flight, List<BigDecimal> flightLengths, BigDecimal minTread,
            OccupancyTypeHelper mostRestrictiveOccupancyType) {
     ...
     * DxfFileConstants.A_AF.equalsIgnoreCase(mostRestrictiveOccupancyType.getSubtype().getCode())) { return
     * BigDecimal.valueOf(0.25); } else { return BigDecimal.valueOf(0.3); } }
     */
    ...
    private void validateNoOfRises(Plan planDetail, HashMap<String, String> errors, Block block,
        ...
        if (!(Boolean) typicalFloorValues.get("isTypicalRepititiveFloor")) {
            ...
            if (valid) {
                setReportOutputDetailsFloorStairWise(planDetail, RULE42_5_II, value,
                        String.format(NO_OF_RISE_DESCRIPTION, fireStair.getNumber(), flight.getNumber()), EXPECTED_NO_OF_RISE,
