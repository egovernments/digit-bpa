...
public class FireStair extends FeatureProcess {
	...
	public Plan process(Plan planDetail) {
		...
		blk: for (Block block : planDetail.getBlocks()) {
			if (block.getBuilding() != null && !block.getBuilding().getOccupancies().isEmpty()
				...
				for (Floor floor : floors) {
					...
					if (fireStairs.size() != 0) {
						...
						for (org.egov.common.entity.edcr.FireStair fireStair : fireStairs) {
							...
							BigDecimal noOfRises = fireStair.getNoOfRises();
									...
									if (flightPolyLineClosed) {
										if (flightWidths != null && flightWidths.size() > 0) {
											minFlightWidth = validateWidth(planDetail, scrutinyDetail2, floor, block,
													typicalFloorValues, fireStair, flightWidths, minFlightWidth,
										...
										if (flightLengths != null && flightLengths.size() > 0) {
											try {
												minTread = validateTread(planDetail, errors, block, scrutinyDetail3,
														floor, typicalFloorValues, fireStair, flightLengths, minTread,
														mostRestrictiveOccupancyType);
										...
										if (noOfRises.compareTo(BigDecimal.ZERO) > 0) {
											try {
												validateNoOfRises(planDetail, errors, block, scrutinyDetailRise, floor,
														typicalFloorValues, fireStair, noOfRises);
											} catch (ArithmeticException e) {
												LOG.info("Denominator is zero");
											}
										} else {
											String layerName = String.format(
													DxfFileConstants.LAYER_FIRESTAIR_FLIGHT_FLOOR, block.getNumber(),
							 ...
							 * boolean valid = false;
							 * 
							 * if (line != null) { BigDecimal lineLength =
							 * Util.roundOffTwoDecimal(line.getLength());
							 * 
							 * if (!(Boolean) typicalFloorValues.get("isTypicalRepititiveFloor")) {
							 * BigDecimal minLineLength = Util.roundOffTwoDecimal(BigDecimal.valueOf(0.75));
							 * 
							 * if (lineLength.compareTo(minLineLength) >= 0) { valid = true; } String value
							 * = typicalFloorValues.get("typicalFloors") != null ? (String)
							 * typicalFloorValues.get("typicalFloors") : " floor " + floor.getNumber();
							 * 
							 * if (valid) setReportOutputDetailsFloorStairWise(planDetail, RULE114, value,
				...
				/*
				 * boolean isAbuting = abutingList.stream().anyMatch(aBoolean -> aBoolean ==
				 * true);
				 * 
				 * if (occupancyType != null) { if
				 * (occupancyType.equalsIgnoreCase("RESIDENTIAL") &&
				 * floorSize.compareTo(BigDecimal.valueOf(3)) > 0) { if (fireStairCount > 0) {
				 ...
				 * scrutinyDetail4); } else { if (spiralStairCount == 0)
				 * setReportOutputDetails(planDetail, RULE42,
				 * String.format(DcrConstants.RULE114, block.getNumber()),
				 * "Minimum 1 fire stair is required", DcrConstants.OBJECTNOTDEFINED_DESC,
				 * Result.Not_Accepted.getResultVal(), scrutinyDetail4); } } else { if
				 * (floorSize.compareTo(BigDecimal.valueOf(2)) > 0) { if (fireStairCount > 0) {
				 * setReportOutputDetails(planDetail, RULE42,
				 * String.format(DcrConstants.RULE114, block.getNumber()), "",
				 * DcrConstants.OBJECTDEFINED_DESC, Result.Accepted.getResultVal(),
				 * scrutinyDetail4); } else { if (spiralStairCount == 0)
				...
				/*
				 * if (fireStairCount > 0) { if (isAbuting) { setReportOutputDetails(planDetail,
				 * RULE114, String.format(DcrConstants.RULE114, block.getNumber()),
		...
		if (!(Boolean) typicalFloorValues.get("isTypicalRepititiveFloor")) {
			...
			if (valid) {
				setReportOutputDetailsFloorStairWise(planDetail, RULE42_5_II, value,
						String.format(WIDTH_DESCRIPTION, fireStair.getNumber()), minimumWidth.toString(),
	...
	private BigDecimal validateTread(Plan planDetail, HashMap<String, String> errors, Block block,
			...
			org.egov.common.entity.edcr.FireStair fireStair, List<BigDecimal> flightLengths, BigDecimal minTread,
			OccupancyTypeHelper mostRestrictiveOccupancyType) {
		...
		if (fireStair.getNoOfRises() != null) {
			...
			if (fireStair.getNoOfRises().compareTo(noOfFlights) > 0) {
				BigDecimal denominator = fireStair.getNoOfRises().subtract(noOfFlights);
				...
				if (!(Boolean) typicalFloorValues.get("isTypicalRepititiveFloor")) {
					...
					if (valid) {
						setReportOutputDetailsFloorStairWise(planDetail, RULE42_5_II, value,
								String.format(TREAD_DESCRIPTION, fireStair.getNumber()), requiredTread.toString(),
	...
	private void validateNoOfRises(Plan planDetail, HashMap<String, String> errors, Block block,
		...
		if (!(Boolean) typicalFloorValues.get("isTypicalRepititiveFloor")) {
			...
			if (valid) {
				setReportOutputDetailsFloorStairWise(planDetail, RULE42_5_II, value,
						String.format(NO_OF_RISE_DESCRIPTION, fireStair.getNumber()), EXPECTED_NO_OF_RISE,
	...
	/*
	 * private void validateDimensions(Plan planDetail, String blockNo, int floorNo,
