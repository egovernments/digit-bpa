...
public class FireStair extends FeatureProcess {
	...
	private static final String WIDTH_LANDING_DESCRIPTION = "Minimum width for fire stair %s landing %s";
	...
	public Plan process(Plan planDetail) {
		...
		blk: for (Block block : planDetail.getBlocks()) {
			...
			if (block.getBuilding() != null) {
				 ...
				 * Util.checkExemptionConditionForSmallPlotAtBlkLevel(planDetail
				 * .getPlot(), block)) { continue blk; }
				 */
					...
					if (!floor.getTerrace()) {
						...
						if (!fireStairs.isEmpty()) {
							for (org.egov.common.entity.edcr.FireStair fireStair : fireStairs) {
								...
								if (!landings.isEmpty()) {
									validateLanding(planDetail, scrutinyDetailLanding, floor, typicalFloorValues,
											fireStair, landings);
								} else {
									errors.put(
				...
				if (block.getBuilding().getIsHighRise() && !fireStairAbsent.isEmpty()) {
				...
				/*
				 * boolean isAbuting = abutingList.stream().anyMatch(aBoolean ->
				 * aBoolean == true); if (occupancyType != null) { if
				 * (occupancyType.equalsIgnoreCase("RESIDENTIAL") &&
				 * floorSize.compareTo(BigDecimal.valueOf(3)) > 0) { if
				 * (fireStairCount > 0) { setReportOutputDetails(planDetail,
				 * RULE42, String.format(DcrConstants.RULE114,
				 * block.getNumber()), "", DcrConstants.OBJECTDEFINED_DESC,
				 * Result.Accepted.getResultVal(), scrutinyDetail4); } else { if
				 * (spiralStairCount == 0) setReportOutputDetails(planDetail,
				 * RULE42, String.format(DcrConstants.RULE114,
				 * block.getNumber()), "Minimum 1 fire stair is required",
				 ...
				 * if (fireStairCount > 0) { setReportOutputDetails(planDetail,
				 * RULE42, String.format(DcrConstants.RULE114,
				 * block.getNumber()), "", DcrConstants.OBJECTDEFINED_DESC,
				 * Result.Accepted.getResultVal(), scrutinyDetail4); } else { if
				 * (spiralStairCount == 0) setReportOutputDetails(planDetail,
				 * RULE42, String.format(DcrConstants.RULE114,
				 * block.getNumber()), "", DcrConstants.OBJECTNOTDEFINED_DESC,
				 * Result.Not_Accepted.getResultVal(), scrutinyDetail4); } } } }
				 */

				/*
				 * if (fireStairCount > 0) { if (isAbuting) {
				 ...
				 * Result.Accepted.getResultVal(), scrutinyDetail7); } else {
				 * setReportOutputDetails(planDetail, RULE114,
				 * String.format(DcrConstants.RULE114, block.getNumber()),
	...
	private void validateLanding(Plan planDetail, ScrutinyDetail scrutinyDetailLanding, Floor floor,
		...
		for (StairLanding landing : landings) {
			...
			if (!(Boolean) typicalFloorValues.get("isTypicalRepititiveFloor")) {
				...
				String value = typicalFloorValues.get("typicalFloors") != null
						? (String) typicalFloorValues.get("typicalFloors") : " floor " + floor.getNumber();
	...
	private void validateFlight(Plan planDetail, HashMap<String, String> errors, Block block,
		...
		for (Flight flight : fireStair.getFlights()) {
			...
			if (!floor.getTerrace()) {
				if (flightPolyLines != null && flightPolyLines.size() > 0) {
					if (flightPolyLineClosed) {
						...
						/*
						 * (Total length of polygons in layer
						 * BLK_n_FLR_i_FIRESTAIR_k_FLIGHT) / (Number of rises -
						 * number of polygons in layer
						 * BLK_n_FLR_i_FIRESTAIR_k_FLIGHT - number of lines in
						 * layer BLK_n_FLR_i_FIRESTAIR_k_FLIGHT)
						 */

						if (flightLengths != null && flightLengths.size() > 0) {
							try {
								minTread = validateTread(planDetail, errors, block, scrutinyDetail3, floor,
						...
						if (noOfRises.compareTo(BigDecimal.ZERO) > 0) {
							...
							errors.put("noofRise" + layerName,
									edcrMessageSource.getMessage(DcrConstants.OBJECTNOTDEFINED,
											new String[] { NO_OF_RISES + layerName }, LocaleContextHolder.getLocale()));
							planDetail.addErrors(errors);
						}
			...
			/*
			 * List<Line> lines = fireStair.getLinesInFlightLayer(); if (lines
			 * != null && lines.size() > 0) { Line line =
			 * lines.stream().min(Comparator.comparing(Line::getLength)).get();
			 * boolean valid = false; if (line != null) { BigDecimal lineLength
			 * = Util.roundOffTwoDecimal(line.getLength()); if (!(Boolean)
			 * typicalFloorValues.get("isTypicalRepititiveFloor")) { BigDecimal
			 * minLineLength =
			 * Util.roundOffTwoDecimal(BigDecimal.valueOf(0.75)); if
			 * (lineLength.compareTo(minLineLength) >= 0) { valid = true; }
			 * String value = typicalFloorValues.get("typicalFloors") != null ?
			 * (String) typicalFloorValues.get("typicalFloors") : " floor " +
			 * floor.getNumber(); if (valid)
			 ...
			 * Result.Accepted.getResultVal(), scrutinyDetail6); else
			 * setReportOutputDetailsFloorStairWise(planDetail, RULE114, value,
			 * String.format(LINE_DESCRIPTION, fireStair.getNumber()),
			 * EXPECTED_LINE, String.valueOf(lineLength),
			 ...
			 * minTread.compareTo(BigDecimal.valueOf(0.3)) >= 0 &&
	 ...
	 * mostRestrictiveOccupancyType) { if
	...
	private BigDecimal validateTread(Plan planDetail, HashMap<String, String> errors, Block block,
		...
		if (flight.getNoOfRises() != null) {
			 ...
			 * fireStair.getNoOfRises().subtract(BigDecimal.valueOf(
			 * flightLengths.size()))
			 * .subtract(BigDecimal.valueOf(fireStair.getLinesInFlightLayer().
			 * size()));
			 */
	 ...
	 * getSubtype().getCode())) { return BigDecimal.valueOf(0.25); } else {
	...
	private void validateNoOfRises(Plan planDetail, HashMap<String, String> errors, Block block,
		...
		if (!(Boolean) typicalFloorValues.get("isTypicalRepititiveFloor")) {
			...
			if (valid) {
				setReportOutputDetailsFloorStairWise(planDetail, RULE42_5_II, value,
						String.format(NO_OF_RISE_DESCRIPTION, fireStair.getNumber(), flight.getNumber()),
	 ...
	 * ruleDesc, String expected, String actual, String status, ScrutinyDetail
	 * scrutinyDetail) { Map<String, String> details = new HashMap<>();
	 * details.put(RULE_NO, ruleNo); details.put(DESCRIPTION, ruleDesc);
	 * details.put(REQUIRED, expected); details.put(PROVIDED, actual);
	...
	/*
	 * private void validateDimensions(Plan planDetail, String blockNo, int
	 * floorNo, String stairNo, List<Measurement> flightPolyLines) { int count =
	 * 0; for (Measurement m : flightPolyLines) { if (m.getInvalidReason() !=
	 * null && m.getInvalidReason().length() > 0) { count++; } } if (count > 0)
