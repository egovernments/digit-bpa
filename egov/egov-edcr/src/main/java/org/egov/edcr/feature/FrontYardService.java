...
public class FrontYardService extends GeneralRule {
					...
					if (setback.getFrontYard() != null) {
						...
						if (buildingHeight != null && (min.doubleValue() > 0 || mean.doubleValue() > 0)) {
							for (final Occupancy occupancy : block.getBuilding().getTotalArea()) {
								...
								if (occupancy.getType().equals(OCCUPANCY_A1)
										|| occupancy.getType().equals(OCCUPANCY_F)) {
									...
									if (buildingHeight.compareTo(BigDecimal.valueOf(10)) <= 0
										...
										checkFrontYardUptoTenMts(pl, block.getBuilding(), block.getName(),
												...
												occupancy.getType(), frontYardResult, errors);
								...
								} else if (occupancy.getType().equals(OCCUPANCY_G1)) {
									checkFrontYardForIndustrial(pl, block.getBuilding(), block.getName(),
											setback.getLevel(), plot, FRONT_YARD_DESC, min, mean, occupancy.getType(),
											frontYardResult);
								} else {
	...
	private Boolean checkFrontYardUptoSixteenMts(SetBack setback, Building building, BigDecimal blockBuildingHeight,
			...
			OccupancyType mostRestrictiveOccupancy, FrontYardResult frontYardResult, HashMap<String, String> errors) {
		Boolean valid = false;
		...
		} else if (mostRestrictiveOccupancy.equals(OCCUPANCY_F)) {
	...
	private Boolean residentialUptoSixteenMts(Integer level, String blockName, BigDecimal min, BigDecimal mean,
			OccupancyType mostRestrictiveOccupancy, FrontYardResult frontYardResult, Boolean valid, String subRule,
	...
	private Boolean checkFrontYardAboveSixteenMts(SetBack setback, Building building, BigDecimal blockBuildingHeight,
			...
			BigDecimal mean, OccupancyType mostRestrictiveOccupancy, FrontYardResult frontYardResult) {
		Boolean valid = false;
	...
	private Boolean checkFrontYardUptoTenMts(Plan pl, Building building, String blockName, Integer level, Plot plot,
			String frontYardFieldName, BigDecimal min, BigDecimal mean, OccupancyType mostRestrictiveOccupancy,
	...
	private Boolean checkFrontYardBasement(Plan planDetail, Building building, String blockName, Integer level,
			...
			OccupancyType mostRestrictiveOccupancy, FrontYardResult frontYardResult) {
		Boolean valid = false;
		String subRule = RULE_47;
		String rule = FRONT_YARD_DESC;
		BigDecimal minVal = BigDecimal.ZERO;
		BigDecimal meanVal = BigDecimal.ZERO;
		if (mostRestrictiveOccupancy.equals(OCCUPANCY_A1) || mostRestrictiveOccupancy.equals(OCCUPANCY_F)) {
	...
	private Boolean checkFrontYardOtherOccupancies(Plan pl, Building building, String blockName, Integer level,
		...
		else if (mostRestrictiveOccupancy.equals(OCCUPANCY_A1)) {
