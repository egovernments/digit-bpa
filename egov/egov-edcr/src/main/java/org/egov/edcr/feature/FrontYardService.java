...
public class FrontYardService extends GeneralRule {
    private static final String RULE_35 = "35";
                        ...
                        if (buildingHeight != null && (min.doubleValue() > 0 || mean.doubleValue() > 0)) {
                            for (final Occupancy occupancy : block.getBuilding().getTotalArea()) {
                                ...
                                if (-1 == setback.getLevel()) {
                                    ...
                                    checkFrontYardBasement(pl, block.getBuilding(), block.getName(),
                                ...
                                if ((occupancy.getTypeHelper().getSubtype() != null
                                        ...
                                        || A_AF.equals(occupancy.getTypeHelper().getSubtype().getCode()))
                                   ...
                                   * else if (G.equals(occupancy.getTypeHelper().getType().getCode())) {
                                   * checkFrontYardForIndustrial(pl, block.getBuilding(), block.getName(), setback.getLevel(),
                                   * plot, FRONT_YARD_DESC, min, mean, occupancy.getTypeHelper(), frontYardResult); } else {
                                   * checkFrontYardOtherOccupancies(pl, block.getBuilding(), block.getName(), setback.getLevel(),
                                   * plot, FRONT_YARD_DESC, min, mean, occupancy.getTypeHelper(), frontYardResult); }
                                   */
                            ...
                            if (errors.isEmpty()) {
                                ...
                                details.put(REQUIRED, frontYardResult.expectedminimumDistance.toString());
    ...
    private Boolean residentialUptoSixteenMts(Integer level, String blockName, BigDecimal min, BigDecimal mean,
            OccupancyTypeHelper mostRestrictiveOccupancy, FrontYardResult frontYardResult, Boolean valid, String subRule,
            String rule, BigDecimal minVal, BigDecimal meanVal, BigDecimal depthOfPlot, HashMap<String, String> errors,
            Plan pl) {
        ...
        /*
         * if (-1 == level) { rule = BSMT_FRONT_YARD_DESC; subRuleDesc = SUB_RULE_24_12_DESCRIPTION; subRule = SUB_RULE_24_12; }
         */
    ...
    private Boolean commercialUptoSixteenMts(Integer level, String blockName, BigDecimal min, BigDecimal mean,
            OccupancyTypeHelper mostRestrictiveOccupancy, FrontYardResult frontYardResult, Boolean valid, String subRule,
            String rule, BigDecimal minVal, BigDecimal meanVal, BigDecimal depthOfPlot) {
        if (depthOfPlot.compareTo(BigDecimal.valueOf(10)) <= 0) {
            minVal = FRONTYARDMINIMUM_DISTANCE_4_5;
        } else if (depthOfPlot.compareTo(BigDecimal.valueOf(10)) > 0
                && depthOfPlot.compareTo(BigDecimal.valueOf(15)) <= 0) {
    ...
    private Boolean checkFrontYardUptoTenMts(Plan pl, Building building, String blockName, Integer level, Plot plot,
        ...
        BigDecimal meanVal = BigDecimal.ZERO;
        BigDecimal depthOfPlot = pl.getPlanInformation().getDepthOfPlot();
        if (mostRestrictiveOccupancy.getSubtype() != null && A_R.equals(mostRestrictiveOccupancy.getSubtype().getCode())
                || A_AF.equals(mostRestrictiveOccupancy.getSubtype().getCode())) {
            valid = residentialUptoTenMts(blockName, level, min, mean, mostRestrictiveOccupancy, frontYardResult, valid,
    ...
    private Boolean checkFrontYardForIndustrial(Plan pl, Building building, String blockName, Integer level, Plot plot,
        ...
        if (mostRestrictiveOccupancy.getType() != null && A_R.equals(mostRestrictiveOccupancy.getType().getCode())) {
            valid = processFrontYardForIndustrial(blockName, level, min, mean, mostRestrictiveOccupancy,
                    frontYardResult, valid, subRule, rule, minVal, meanVal, pl.getPlot().getArea(), widthOfPlot);
        }
        return valid;
    }
    ...
    private Boolean checkFrontYardOtherOccupancies(Plan pl, Building building, String blockName, Integer level,
        ...
        else if (mostRestrictiveOccupancy.equals(OCCUPANCY_B3)) {
            minVal = FRONTYARDMINIMUM_DISTANCE_9;
        ...
        else if (mostRestrictiveOccupancy.equals(OCCUPANCY_D1)) {
            minVal = FRONTYARDMINIMUM_DISTANCE_12;
            subRule = RULE_37_TWO_D;
        } // Hazardous
        else if (mostRestrictiveOccupancy.equals(A_R)) {
    ...
    private Boolean checkFrontYardUptoTwelveMts(SetBack setback, Building building, Plan pl, Integer level,
        ...
        if (A_R.equals(mostRestrictiveOccupancy.getSubtype().getCode()) || A_AF.equals(mostRestrictiveOccupancy.getSubtype().getCode())) {
    ...
    private Boolean processFrontYardForIndustrial(String blockName, Integer level, BigDecimal min, BigDecimal mean,
            OccupancyTypeHelper mostRestrictiveOccupancy, FrontYardResult frontYardResult, Boolean valid, String subRule,
            String rule, BigDecimal minVal, BigDecimal meanVal, BigDecimal plotArea, BigDecimal widthOfPlot) {
