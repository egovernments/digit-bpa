...
public class ReceiptHeaderService extends PersistenceService<ReceiptHeader, Long> {
	...
	/**
	 * @param statusCode  Status code of receipts to be fetched. If null or ALL,
	 *                    then receipts with all statuses are fetched
	 * @param userName    User name of the user who has created the receipts. If
	 *                    null or ALL, then receipts of all users are fetched
	 * @param counterId   Counter id on which the receipts were created. If
	 *                    negative, then receipts from all counters are fetched
	 * @param serviceCode Service code for which the receipts were created. If null
	 *                    or ALL, then receipts of all billing services are fetched
	 * @return List of all receipts created by given user from given counter id and
	 *         having given status
	 */
	 ...
	 *
	 * @param receiptHeader     Receipt header for which the pre-approval voucher is
	 *                          to be created
	...
	protected CVoucherHeader createVoucher(final ReceiptHeader receiptHeader) {
		...
		/**
		 * Aggregate Amount in case of bill based receipt for account codes appearing
		 * more than once in receipt details
		 */
	 ...
	 * following: 1. Start workflow 2. Transition workflow state with action "Create
	 * Receipt" 3. Create vouchers (if required) 4. If vouchers created, transition
	 * workflow state with action "Create Voucher"
	 *
	 * @param receiptHeaders    set of receipt headers on which workflow is to be
	 *                          started
	 * @param receiptBulkUpload
	 */
	public void startWorkflow(final ReceiptHeader receiptHeader) {
		...
		if (receiptHeader.getState() == null && !createVoucherForBillingService)
			receiptHeader.transition().start()
					.withSenderName(this.securityUtils.getCurrentUser().getUsername() + "::"
		...
		for (int m = 0; m < paramList.size(); m++) {
			...
			if (arrayObjectInitialIndexTemp[1] != null && arrayObjectInitialIndexTemp[2] != null)
				if (arrayObjectInitialIndexTemp[1]
						.equals(objHashMapTemp.get(CollectionConstants.BANKREMITTANCE_RECEIPTDATE))
						&& arrayObjectInitialIndexTemp[2]
								.equals(objHashMapTemp.get(CollectionConstants.BANKREMITTANCE_RECEIPTNUMBER))
						&& arrayObjectInitialIndexTemp[11]
								.equals(objHashMapTemp.get(CollectionConstants.BANKREMITTANCE_INSTRUMENTID))) {
					check = m;
					break;
				} else
					continue;
	...
	/**
	 * API to check if the given receipt detail object already exists in the list
	 * passed passed as parameter
	 *
	...
	public void endReceiptWorkFlowOnCancellation(final ReceiptHeader receiptHeaderToBeCancelled) {
		...
		receiptHeaderToBeCancelled.transition().end()
	...
	/**
	 * This method persists the given <code>ReceiptPayeeDetails</code> entity. The
	 * receipt number for all of the receipts is generated, if not already present.
	 * If the receipt is associated with a challan, and the challan number is not
	 * present, the challan number is generated and set into it.
	 */
	@Override
	@Transactional
	...
	/**
	 * This method persists the given <code>ReceiptPayeeDetails</code> entity. If
	 ...
	 * @return void @ Create instrument voucher list from receiptpayeelist and pass
	 *         it to financialsutil
	 */
	...
	public void performWorkflow(final String actionName, final ReceiptHeader receiptHeader, final String remarks) {
		try {
			...
			final List<Assignment> assignments = assignmentService
					.getAllActiveEmployeeAssignmentsByEmpId(emp.get(0).getId());
	...
	public void perform(final ReceiptHeader receiptHeader, final String wfState, final String newStatusCode,
		...
		if (!collectionsUtil.isEmployee(this.securityUtils.getCurrentUser()))
			loggedInUserposition = collectionsUtil
					.getPositionByDeptDesgAndBoundary(receiptHeader.getReceiptMisc().getBoundary());
		else
		...
		if (loggedInUserposition != null && receiptHeader.getState() != null
			...
			throw new ValidationException(Arrays
					.asList(new ValidationError("Current user is not the owner of the selected workflow inbox item",
		...
		if (receiptHeader.getStatus().getCode().equals(CollectionConstants.RECEIPT_STATUS_CODE_APPROVED))
			...
			receiptHeader.transition().progressWithStateCopy()
					.withSenderName(
							receiptHeader.getCreatedBy().getUsername() + "::" + receiptHeader.getCreatedBy().getName())
	...
	public String getReceiptHeaderforDishonor(final String instrumentType, final Long bankAccId, final Long bankId,
		...
		queryString.append(
				...
				.append("AND status.description = '").append(CollectionConstants.INSTRUMENT_DEPOSITED_STATUS + "'))");
	 ...
	 * list are not empty, the .... is updated The receipt ids of the newly created
	 * receipts are collectively populated to be shown on the print screen
	 */
	@Transactional
	 ...
	 * @param status        a <code>EgwStatus</code> instance representing the state
	 *                      to which the receipt has to be updated with
	...
	/**
	 * No work flow before cut off Date OR logged in user is Bank Collection
	 * Operator else Start work flow for newly created receipt.
	 */
	...
	public void validatePayment(String serviceCode, String billReferenceNumber, BigDecimal actualAmountPaid,
			ArrayList<ReceiptDetail> receiptDetailsArray) {
		BillingIntegrationService billingService = getBillingServiceBean(serviceCode);
		try {
			billingService.validatePayment(billReferenceNumber, actualAmountPaid, receiptDetailsArray);
		} catch (ApplicationRuntimeException e) {
			LOGGER.error(" ", e);
			throw new ValidationException(
					new ValidationError("paymentError", "Problem in payment. Please contact adminstrator"));
		} catch (final Exception e) {
			LOGGER.error("", e);
			throw new ValidationException(new ValidationError("paymentAmtValidationMsg",
					"Error while validating the payment amount with billing system."));
		}
	}
}