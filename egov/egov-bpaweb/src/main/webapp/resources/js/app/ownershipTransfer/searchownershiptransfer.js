...
$(document)
		.ready(
				function() {
					...
					var viewurl = '/bpa/application/ownership/transfer/view/';
					var collectfeeurl = '/bpa/application/ownership/transfer/generate-bill/';
					var ownershiporderurl = '/bpa/application/ownership/transfer/generateorder/';
					var rejectionnoticeurl = '/bpa/application/ownership/transfer/rejectionnotice/';
					$('#btnSearch').click(function() {
						var isValid = false;
                        $('#searchOwnershipForm').find(':input',':select',':textarea').each(function() {
                            if($(this).val()) {
                                isValid =true;
                                return false;
							} else
								...
								.dataTable(
										{
                                            ...
                                            "order": [[1, 'asc']],
											ajax : {
												url : "/bpa/application/ownership/transfer/search",
												type : "POST",
												beforeSend : function() {
													$('.loader-class')
															.modal(
													...
													},
													{
													...
													},
					          						{
														...
														"render" : function(
															...
															if (row.status == 'Approved' && row.isFeeCollected && row.feeCollector) {
																return ('<select class="dropchange" style="width:160px;font-size: small">'+commonOptions+'<option  value='
																		+ collectfeeurl
																		+ row.applicationNumber + '>Pay Ownership transfer Fee</option></select>');
															} else if (row.status == 'Submitted' && row.isFeeCollected && row.feeCollector) {
																return ('<select class="dropchange" style="width:160px;font-size: small">'+commonOptions+'<option  value='
																		+ collectfeeurl
																		+ row.applicationNumber + '>Pay Ownership transfer application Fee</option></select>');
															} else if (row.status == 'Approved' && row.isFeeCollected && !row.feeCollector) {
																return ('<select class="dropchange" style="width:160px;font-size: small">'+commonOptions+'></select>');

															} else if (row.status == 'Rejected' && row.pendingAction == 'END') {
																return ('<select class="dropchange" style="width:160px;font-size: small">'+commonOptions+'<option  value='
																		+ rejectionnoticeurl
																...
																return ('<select class="dropchange" style="width:160px;font-size: small">'+commonOptions+'<option  value='
																		+ ownershiporderurl
																		+ row.applicationNumber + '>Generate Ownership Transfer Order</option></select>');
															} else {
																return ('<select class="dropchange" style="width:160px;font-size: small">'+commonOptions+'></select>');
															}
														}
