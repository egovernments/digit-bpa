...
jQuery(document).ready(function ($) {
    ...
    var validator = $("#updateOwnershipApplicationForm").validate({
    ...
    var tbody = $('#ownershipAdditionalRejectionReasons').children('tbody');
    var table = tbody.length ? tbody : $('#ownershipAdditionalRejectionReasons');
    var row = '<tr>' +
        ...
        +'<input type="hidden" name="additionalOwnershipConditionsTemp[{{idx}}].noticeCondition.checklistServicetype" value="{{ownershipConditionId}}" />'
        +'<input type="hidden" class="additionalOwnershipCondition" name="additionalRejectReasonsTemp[{{idx}}].noticeCondition.type" value="ADDITIONALREJECTIONREASONS"/>'
        +'<input type="hidden" class="serialNo" data-sno name="additionalRejectReasonsTemp[{{idx}}].noticeCondition.orderNumber"/></td>' 
        +'<td><textarea class="form-control patternvalidation additionalOwnershipCondition" data-pattern="alphanumericspecialcharacters" rows="2" maxlength="500" name="additionalRejectReasonsTemp[{{idx}}].noticeCondition.additionalCondition"/></td>';
    $('#addAddnlRejectRow').click(function () {
        var idx = $(tbody).find('tr').length;
        if(validateAdditionalConditionsOrReasonsOnAdd('ownershipAdditionalRejectionReasons')) {
        	//Add row
            var row = {
                'sno': idx + 1,
                'idx': idx,
                'ownershipConditionId': $('#additionalOwnershipCondition').val(),
    ...
    function validateAdditionalConditionsOrReasonsOnAdd(tableId){
        ...
        $('#'+tableId+' tbody tr').each(function(index){
        	var additionalOwnershipCondition  = $(this).find('*[name$="additionalCondition"]').val();
                            ...
                            buttons: {
                                'confirm': {
                                    ...
                                    callback: function (result) {
                                        $('#updateOwnershipApplicationForm').trigger('submit');
                                    }
...
function validateForm(validator) {
    if ($('#updateOwnershipApplicationForm').valid()) {
        return true;
    } else {
