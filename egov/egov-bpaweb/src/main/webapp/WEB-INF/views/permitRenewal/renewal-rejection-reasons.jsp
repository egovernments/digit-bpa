...
<div class="panel-body rejectionReason display-hide">
	<table class="table table-bordered  multiheadertbl"
		...
		<tbody>
		<c:choose>
			<c:when test="${not empty permitRenewal.rejectionReasonsTemp}">
				<c:forEach var="rejectionReason" items="${permitRenewal.rejectionReasonsTemp}"
						   varStatus="rejectPCStatus">
					<tr>
						<td class="text-center"><form:hidden
								path="rejectionReasonsTemp[${rejectPCStatus.index}].permitRenewal" value="${permitRenewal.id}" /> 
								...
								<form:hidden path="rejectionReasonsTemp[${rejectPCStatus.index}].noticeCondition.type" value="RENEWALREJECTIONREASONS" /> 
	...
	<table class="table table-bordered  multiheadertbl"
		   id="renewalAdditionalRejectionReasons">
		<thead>
		<tr>
			<th><spring:message code="lbl.srl.no" /></th>
			<th><spring:message code="lbl.select" /></th>
		...
		<tbody>
		<c:choose>
			<c:when	test="${not empty permitRenewal.additionalRejectReasonsTemp}">
				<c:forEach var="addnlPermitCondition"
						   items="${permitRenewal.additionalRejectReasonsTemp}"
						   varStatus="addnlPCStatus">
					<tr>
						<td class="text-center"><form:hidden
								path="additionalRejectReasonsTemp[${addnlPCStatus.index}].permitRenewal"
