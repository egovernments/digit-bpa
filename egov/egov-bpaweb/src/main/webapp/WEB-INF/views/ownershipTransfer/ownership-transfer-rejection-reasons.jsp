...
<div class="panel-body rejectionReason display-hide">
	<table class="table table-bordered  multiheadertbl"
		   id="ownershipAdditionalRejectionReasons">
		<thead>
		<tr>
		...
		<tbody>
		<c:choose>
			<c:when test="${not empty ownershipTransfer.rejectionReasonsTemp}">
				<c:forEach var="rejectionReason" items="${ownershipTransfer.rejectionReasonsTemp}"
						   varStatus="rejectPCStatus">
					<tr>
						<td class="text-center"><form:hidden
								path="rejectionReasonsTemp[${rejectPCStatus.index}].ownershipTransfer" value="${ownershipTransfer.id}" /> 
								...
								<form:hidden path="rejectionReasonsTemp[${rejectPCStatus.index}].noticeCondition.type" value="OWNERSHIPREJECTIONREASONS" /> 
	...
	<table class="table table-bordered  multiheadertbl"
		...
		<tbody>
		<c:choose>
			<c:when	test="${not empty ownershipTransfer.additionalRejectReasonsTemp}">
				<c:forEach var="addnlOwnershipCondition"
						   items="${ownershipTransfer.additionalRejectReasonsTemp}"
						   varStatus="addnlPCStatus">
					<tr>
						<td class="text-center"><form:hidden
								path="additionalRejectReasonsTemp[${addnlPCStatus.index}].ownershipTransfer"
							...
							<form:hidden id="additionalPermitCondition"
										 ...
										 value="${addnlOwnershipCondition.noticeCondition.checklistServicetype.id}"/>
							<form:hidden
									...
									value="${addnlOwnershipCondition.noticeCondition.orderNumber}" />
