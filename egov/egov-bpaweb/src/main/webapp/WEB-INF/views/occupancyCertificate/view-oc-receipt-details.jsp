...
<div class="panel-body display-hide">
    <table class="table table-striped table-bordered" id="bpaReceiptDetail" style="width:50%;margin:0 auto;">
        ...
        <tbody>
        <c:choose>
            <c:when test="${not empty  occupancyCertificate.receipts}">
                <c:set var="totalAmount" value="${0}"/>
                <c:forEach var="receipt" items="${occupancyCertificate.receipts}"
                           varStatus="counter">
                    <tr>
                        <td class="text-center view-content"><c:out value="${counter.index+1}"/></td>
                        <td class="view-content"><a class="open-popup"
                                                    href="/collection/citizen/onlineReceipt-viewReceipt.action?receiptNumber=${receipt.receiptNumber}&consumerCode=${occupancyCertificate.applicationNumber}&serviceCode=BPA">${receipt.receiptNumber}</a>
                        </td>
