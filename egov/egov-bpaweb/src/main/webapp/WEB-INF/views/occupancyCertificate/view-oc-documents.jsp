...
<div class="panel-body">
	<table class="table table-bordered  multiheadertbl">
		...
		<tbody>
			<c:choose>
				<c:when test="${not empty  occupancyCertificate.getDocuments()}">
					<c:forEach items="${occupancyCertificate.getDocuments()}" var="docs"
						varStatus="status">
						<tr>
							...
							<td class="view-content" style="font-size: 97%;"><c:set value="false" var="isDocFound"></c:set> <c:forEach
									var="ocDoc" items="${docs.document.getOrderedSupportDocs()}" varStatus="loop">
									<c:if test="${ocDoc.fileStoreId ne null}">
										<c:set value="true" var="isDocFound"></c:set>
										<a target="_blank" href="/bpa/application/downloadfile/${ocDoc.fileStoreId}"
											data-gallery>${loop.index +1} - ${ocDoc.fileName} </a>
