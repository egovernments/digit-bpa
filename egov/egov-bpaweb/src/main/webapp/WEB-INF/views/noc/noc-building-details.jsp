...
<div class="panel-body display-hide">
	<c:if test="${not empty noc.bpaApplication.buildingDetail}">
		<c:forEach items="${noc.bpaApplication.buildingDetail}" var="bldg"
			...
			<c:if test="${not empty bldg.applicationFloorDetails}">
				<div
					...
					<div class="panel-title">
						<c:if test="${fn:length(noc.bpaApplication.buildingDetail) gt 1}">Block - ${bldg.name}</c:if>
						<spring:message code="lbl.plint.carpet.details" />
					</div>
					<c:if test="${fn:length(noc.bpaApplication.buildingDetail) gt 1}">
				...
				<div
					<c:if
                    test="${fn:length(noc.bpaApplication.buildingDetail) gt 1}">class="buildingDetailsToggle${bldgCounter.index} display-hide" </c:if>>
						...
						<thead>
							<tr>
								...
								<c:if test="${isEDCRIntegrationRequire eq false}">
									<th class="text-center"><spring:message code="lbl.usage" /></th>
								</c:if>
								<th class="text-center"><spring:message
						...
						<tbody>
							...
							<c:forEach items="${bldg.applicationFloorDetails}"
								...
								<tr class="data-fetched">
									...
									<td class="view-content"><c:out
											value="${floorDetails.subOccupancy.name}" default="N/A"></c:out>
										<c:if test="${isEDCRIntegrationRequire eq false}">
											<td class="view-content"><c:out
													value="${floorDetails.usage.name}" default="N/A"></c:out>
										</c:if>
			...
			<c:if test="${bldg.additionalFeePaymentAccepted}">
				<div class="row add-border">
					...
					<div class="col-sm-3 add-margin view-content">
						<c:out value="${bldg.additionalFeePaymentAccepted ? 'YES' : 'NO'}"></c:out>
					</div>
				</div>
			</c:if>
