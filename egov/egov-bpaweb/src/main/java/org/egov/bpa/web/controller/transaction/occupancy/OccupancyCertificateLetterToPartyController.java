...
public class OccupancyCertificateLetterToPartyController {
	...
	public String createLetterToParty(@ModelAttribute final OCLetterToParty ocLetterToParty,
		...
		if (!existingLetterToParties.isEmpty()) {
			...
			if (existingLpParty.getCreatedBy().equals(securityUtils.getCurrentUser())
				&& existingLpParty.getLetterToParty().getCurrentApplnStatus().equals(ocLetterToParty.getLetterToParty().getCurrentApplnStatus())) {
				ocLetterToParty.getLetterToParty().setCurrentStateValueOfLP(existingLpParty.getLetterToParty().getCurrentStateValueOfLP());
				ocLetterToParty.getLetterToParty().setStateForOwnerPosition(existingLpParty.getLetterToParty().getStateForOwnerPosition());
	...
	public String editLetterToParty(@PathVariable final String applicationNumber, @PathVariable final String lpNumber, final Model model) {
	...
	private void prepareLetterToParty(String applicationNumber, String lpNumber, Model model) {
		...
		model.addAttribute(CHECK_LIST_DETAIL_LIST, getCheckListDetailList(ocLetterToParty.getOc().getParent().getServiceType().getId()));
		model.addAttribute(BPA_APPLICATION, ocLetterToParty.getOc().getParent());
	}

	@PostMapping("/update/{applicationNumber}/{lpNumber}")
	public String updateLetterToParty(@Valid @ModelAttribute("ocLetterToParty") final OCLetterToParty ocLetterToParty, @PathVariable final String applicationNumber,
									  @PathVariable final String lpNumber, final RedirectAttributes redirectAttributes) {
		processAndStoreLetterToPartyDocuments(ocLetterToParty);
		OCLetterToParty ocLtpRes = ocLetterToPartyService.save(ocLetterToParty, ocLetterToParty.getOc().getState().getOwnerPosition().getId());
	...
	public String resultLetterToParty(@PathVariable final String applicationNumber, @PathVariable final String lpNumber, final Model model) {
		OCLetterToParty ocLetterToParty = ocLetterToPartyService.findByOcApplicationNoAndInspectionNo(applicationNumber, lpNumber);
		model.addAttribute(OC_LETTER_TO_PARTY, ocLetterToParty);
		model.addAttribute(LETTER_TO_PARTY_DOC_LIST, ocLetterToParty.getLetterToParty().getLetterToPartyDocuments());
	...
	protected void processAndStoreLetterToPartyDocuments(final OCLetterToParty ocLetterToParty) {
		if (!ocLetterToParty.getLetterToParty().getLetterToPartyDocuments().isEmpty())
			for (final LetterToPartyDocumentCommon lpDocumentCommon : ocLetterToParty.getLetterToParty().getLetterToPartyDocuments()) {
				...
				if (lpDocumentCommon.getFiles() != null && lpDocumentCommon.getFiles().length > 0) {
					...
					lpDocumentCommon.setSubmitted(true);
				}
			}
	}
	...
	public String captureSentDate(@PathVariable final String applicationNumber, @PathVariable final String lpNumber, final Model model) {
		OCLetterToParty ocLetterToParty = ocLetterToPartyService.findByOcApplicationNoAndInspectionNo(applicationNumber, lpNumber);
		model.addAttribute(OC_LETTER_TO_PARTY, ocLetterToParty);
		model.addAttribute(LETTER_TO_PARTY_DOC_LIST, ocLetterToParty.getLetterToParty().getLetterToPartyDocuments());
		model.addAttribute(BPA_APPLICATION, ocLetterToParty.getOc().getParent());
	...
	public String showLetterToPartyReply(@PathVariable final String applicationNumber, @PathVariable final String lpNumber, final Model model) {
		OCLetterToParty ocLetterToParty = ocLetterToPartyService.findByOcApplicationNoAndInspectionNo(applicationNumber, lpNumber);
		model.addAttribute(OC_LETTER_TO_PARTY, ocLetterToParty);
		model.addAttribute(LETTER_TO_PARTY_DOC_LIST, ocLetterToParty.getLetterToParty().getLetterToPartyDocuments());
		model.addAttribute(BPA_APPLICATION, ocLetterToParty.getOc().getParent());
	...
	public String createLetterToPartyReply(@ModelAttribute("ocLetterToParty") final OCLetterToParty ocLetterToParty, @PathVariable final String applicationNumber,
										   @PathVariable final String lpNumber, final RedirectAttributes redirectAttributes) {
		processAndStoreLetterToPartyDocuments(ocLetterToParty);
		OCLetterToParty ocLetterToPartyRes = ocLetterToPartyService.save(ocLetterToParty, ocLetterToParty.getOc().getState().getOwnerPosition().getId());
