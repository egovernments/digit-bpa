...
public class ModuleRepositoryImpl implements ModuleRepositoryCustom {
    ...
    public List<Object[]> fetchModulesForRoles(final Set<Role> roles) {
        final StringBuilder sql = new StringBuilder().append(
                "SELECT DISTINCT mod.name,mod.contextRoot,mod.displayName, mod.id, mod.ordernumber FROM eg_module mod,eg_action act ")
        ...
        return entityManager.createNativeQuery(sql.toString()).setParameter("roles", roles).getResultList();
    }
    ...
    public List<Object[]> fetchModulesByParentModuleId(final Long parentId, final Long userId) {
        ...
        return entityManager.createNativeQuery(sql.toString()).setParameter("parentId", parentId).setParameter("userId", userId)
                .getResultList();
    }
    ...
    public List<Object[]> fetchUserFavourateModules(final Long userId) {
        ...
        return entityManager.createNativeQuery(sql.toString()).setParameter("userId", userId).getResultList();
    }
}
