...
public class ChecklistJsonAdaptor implements JsonSerializer<SearchChecklist> {
    ...
    @Override
    public JsonElement serialize(final SearchChecklist checklist, final Type type, final JsonSerializationContext jsc) {
        final JsonObject jsonObject = new JsonObject();
        if (checklist != null) {
            if (checklist.getChecklistType().getDescription() != null)
                jsonObject.addProperty("checklistType", checklist.getChecklistType().getDescription());
            else
                jsonObject.addProperty("checklistType", "");

            if (checklist.getDescription() != null)
                jsonObject.addProperty("description", checklist.getDescription());
            else
