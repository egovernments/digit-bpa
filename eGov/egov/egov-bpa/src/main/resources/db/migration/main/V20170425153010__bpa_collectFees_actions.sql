...
values (nextval('SEQ_EG_ACTION'),'BpaCollectFees','/application/bpageneratebill/',null,
(select id from eg_module where name='BPA Transanctions'),1,'BpaCollectFees','false','bpa',0,1,now(),1,now(),
(select id from eg_module where name='BPA'));



Insert into eg_roleaction (roleid,actionid) values ((select id from eg_role where name='Collection Operator'),
(select id from eg_action where name='BpaCollectFees'));
