...
public class WorkflowHistoryService {
	...
	public List<HashMap<String, Object>> getHistory(List<BpaAppointmentSchedule> appointmentSchedules, State<Position> state, List<StateHistory<Position>> stateHistories) {
		...
		if (null != workflowState) {
			...
			buildEmployeeInformation(state, stateHistories, workFlowHistory, state != null, state.getValue(), workflowState.getOwnerPosition(), workflowState.getLastModifiedDate(), workflowState.getOwnerUser());
	...
	private void buildEmployeeInformation(State<Position> state, List<StateHistory<Position>> stateHistories, HashMap<String, Object> workFlowHistory, boolean b, String value, Position ownerPosition2, Date lastModifiedDate, User ownerUser) {
		User userObject;
		if (b
			...
			Position scrutinizedByPos = positionMasterService.getPositionById(bpaWorkFlowService.getDocumentScrutinizedUserPosition(state, stateHistories));
			...
			setEmployeeDetailsByDate(userObject, workFlowHistory, scrutinizedByPos == null ? ownerPosition2 : scrutinizedByPos, lastModifiedDate);
		} else {
	...
	private void buildStateHistory(List<StateHistory<Position>> stateHistories, List<HashMap<String, Object>> historyTable, State<Position> workflowState) {
		for (final StateHistory<Position> stateHistory : stateHistories) {
			...
			buildEmployeeInformation(workflowState, stateHistories, historyMap, stateHistory != null, stateHistory.getValue(), stateHistory.getOwnerPosition(), stateHistory.getLastModifiedDate(), stateHistory.getOwnerUser());
			historyTable.add(historyMap);
		}
	}

	public List<HashMap<String, Object>> getHistoryForOC(List<OCAppointmentSchedule> appointmentSchedules, State<Position> state, List<StateHistory<Position>> stateHistories) {
		final List<HashMap<String, Object>> historyTable = new ArrayList<>();
		final State<Position> workflowState = state;
		final HashMap<String, Object> workFlowHistory = new HashMap<>(0);
		if (null != workflowState) {
	...
	private void setEmployeeDetailsByDate(User userObject, HashMap<String, Object> historyMap, Position owner , Date date) {
		...
		if (null != userObject && owner == null) {
			...
			List<Assignment> assignments = bpaWorkFlowService.getAssignmentByPositionAndUserAsOnDate(owner.getId(), userObject.getId(), date);
			if(assignments.isEmpty())
	...
	private void buildApplicationHistoryForSchedulingAppointments(final List<BpaAppointmentSchedule> appointmentSchedules,
		...
		if (!appointmentSchedules.isEmpty()) {
			for (BpaAppointmentSchedule appointmentSchedule : appointmentSchedules) {
				buildSchedulingDetails(historyTable, appointmentSchedule.getPurpose(), appointmentSchedule.getCreatedDate(), appointmentSchedule.isPostponed(), appointmentSchedule.getPostponementReason(), appointmentSchedule.getLastModifiedBy(), appointmentSchedule.getCreatedBy(), appointmentSchedule.getLastModifiedDate());
			}
		}
	}

	private void buildSchedulingDetails(List<HashMap<String, Object>> historyTable, AppointmentSchedulePurpose purpose, Date createdDate, boolean postponed, String postponementReason, User lastModifiedBy, User createdBy, Date lastModifiedDate) {
	...
	private void buildOCApplnHistoryForSchedulingAppointments(final List<OCAppointmentSchedule> appointmentSchedules,
		...
		if (!appointmentSchedules.isEmpty()) {
			for (OCAppointmentSchedule ocAppointmentSchedule : appointmentSchedules) {
				...
				buildSchedulingDetails(historyTable, appmntScheduleCommon.getPurpose(), ocAppointmentSchedule.getCreatedDate(), appmntScheduleCommon.isPostponed(), appmntScheduleCommon.getPostponementReason(), ocAppointmentSchedule.getLastModifiedBy(), ocAppointmentSchedule.getCreatedBy(), ocAppointmentSchedule.getLastModifiedDate());
			}
		}
	}
	...
	public Map<String, String> checkIsEdcrUsedInBpaApplication(final String eDcrNumber) {
		...
		if (bpaApplications.isEmpty() || !bpaApplications.isEmpty() && null == bpaApplications.get(0).getState() && BpaConstants.APPLICATION_STATUS_CANCELLED.equals(bpaApplications.get(0).getStatus().getCode())) {
			...
			String message = bpaMessageSource.getMessage("msg.dcr.exist.with.appln",
					new String[]{securityUtils.getCurrentUser().getName(), bpaApplications.get(0).geteDcrNumber(), bpaApplications.get(0).getApplicationNumber()},
					null);
