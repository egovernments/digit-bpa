...
jQuery(document).ready(function($) {
    ...
    var validator=$("#ocLettertoPartyReplyform").validate({
    ...
    $("#btnLPReply").click(function(e){
        if ($('#ocLettertoPartyReplyform').valid() && validateUploadFilesMandatory()) {
            return true;
        } else {
	...
	function getUrlToPring() {
		var url = '/bpa/occupancy-certificate/letter-to-party/print/lp/' + $('#applicationNumber').val() + '/' + $('#lpNumber').val();
		$('#ocLettertoPartyform').attr('method', 'get');
		$('#ocLettertoPartyform').attr('action', url);
		window.location = url;
	}
