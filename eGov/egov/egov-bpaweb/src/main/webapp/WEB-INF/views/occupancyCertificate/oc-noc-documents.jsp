    ...
    <table class="table table-bordered  multiheadertbl" id=bpaupdatenocdetails>
        ...
        <c:choose>
            <c:when test="${occupancyCertificate.parent.serviceType ne null}">
                <c:forEach var="doc" items="${occupancyCertificate.getNocDocuments()}"
                           varStatus="status">
                    <form:hidden
                            path="nocDocuments[${status.index}].oc"
                            id="nocDocuments${status.index}.oc" value="${occupancyCertificate.id}"/>
                    <form:hidden id="nocDocuments${status.index}.id"
                                 path="nocDocuments[${status.index}].id"
                                 value="${doc.id}"/>
                    <tr>
                        <td><c:out value="${status.index+1}"></c:out></td>
                        <td><c:out value="${doc.nocDocument.checklist.description}"></c:out> <c:if
                                test="${doc.nocDocument.checklist.isMandatory}">
                        ...
                        <td class="hide"><form:input class="form-control datepicker replyReceivedOn" maxlength="50"
                            ...
                            <form:errors
                                    path="nocDocuments[${status.index}].nocDocument.replyReceivedOn"
                                    cssClass="add-margin error-msg"/></td>
                        <td class="hide"><form:select path="nocDocuments[${status.index}].nocDocument.nocStatus"
                            ...
                            <form:errors path="nocDocuments[${status.index}].nocDocument.nocStatus"
                        ...
                        <td>
                            <div class="input-group">
                                <form:textarea
                                        ...
                                        data-input-element="nocDocuments${status.index}remarks" maxlength="1000"
                                        id="nocDocuments${status.index}remarks"
                        ...
                        <td>
                            <div class="files-upload-container"
                                 ...
                                 <c:if test="${docs.nocDocument.checklist.isMandatory eq true && fn:length(docs.nocDocument.getNocSupportDocs()) eq 0}">required</c:if>
                                 data-allowed-extenstion="doc,docx,xls,xlsx,rtf,pdf,txt,zip,jpeg,jpg,png,gif,tiff">
                                <div class="files-viewer">

                                    <c:forEach items="${doc.nocDocument.getNocSupportDocs()}" var="file" varStatus="status1">
